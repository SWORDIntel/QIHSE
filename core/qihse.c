...
static int qihse_execute_cpu_scalar(
    ...
    const float* query_real, const float* query_imag, float* output_scores);
...
static int qihse_compute_partition_amplitudes(
    ...
    switch (partition->device) {
        case QIHSE_DEV_CPU_AMX:
            return qihse_execute_cpu_amx(partition, query_real, query_imag, output_scores);

        case QIHSE_DEV_CPU_VNNI:
            return qihse_execute_cpu_vnni(partition, query_real, query_imag, output_scores);

        case QIHSE_DEV_CPU_AVX512:
            return qihse_execute_cpu_avx512(partition, query_real, query_imag, output_scores);

        case QIHSE_DEV_CPU_AVX2:
            return qihse_execute_cpu_avx2(partition, query_real, query_imag, output_scores);

        default:
            /* Fallback to scalar computation */
            return qihse_execute_cpu_scalar(partition, query_real, query_imag, output_scores);
    }
}
...
not_stisla_result_t qihse_search(
    ...
    static qihse_compute_pool_t* compute_pool = NULL;
    if (!compute_pool) {
        compute_pool = qihse_compute_pool_init();
        if (!compute_pool) {
            return NOT_STISLA_NOT_FOUND;
        }

        /* Calibrate devices */
        qihse_compute_pool_calibrate(compute_pool);
    }
    ...
    qihse_rff_kernel_t* rff_kernel = qihse_rff_create(
    ...
    double* rff_data = malloc(n * optimal_dims * sizeof(double));
    if (!rff_data) {
    ...
    double query_double;
    if (qihse_convert_to_double(query, config->data_type, &query_double) != 0) {
        free(rff_data);
        qihse_rff_destroy(rff_kernel);
        return NOT_STISLA_NOT_FOUND;
    }
    ...
    float* real_float = malloc(n * optimal_dims * sizeof(float));
    float* imag_float = malloc(n * optimal_dims * sizeof(float));
    ...
    double query_rff[optimal_dims];
    qihse_rff_project(rff_kernel, &query_double, query_rff);

    float* query_real = malloc(optimal_dims * sizeof(float));
    float* query_imag = malloc(optimal_dims * sizeof(float));
    ...
    qihse_work_schedule_t* schedule = qihse_create_work_schedule(compute_pool, n, optimal_dims);
    if (!schedule) {
        free(query_imag);
        free(query_real);
        free(imag_float);
    ...
    float* amplitude_scores = calloc(n, sizeof(float));
    if (!amplitude_scores) {
        qihse_work_schedule_destroy(schedule);
        free(query_imag);
        free(query_real);
        free(imag_float);
        free(real_float);
    ...
    for (size_t p = 0; p < schedule->partition_count; p++) {
        ...
        if (qihse_compute_partition_amplitudes(partition, &superposition,
                                              ...
                                              (float*)partition->output_buffer) != 0) {
    ...
    not_stisla_result_t final_result = qihse_verify_result(
