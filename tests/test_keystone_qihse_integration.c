...
static void test_keystone_node_cap_cluster_bus_integration(void) {
    printf("Testing KEYSTONE NODE_CAP Frame Integration into QIHSE Cluster Bus...\n");

    /* Initialize fabric index to bind libkeystone.so */
    assert(qihse_fabric_index_init(NULL, NULL) == QIHSE_FABRIC_INDEX_OK);
    assert(qihse_fabric_index_is_available());

    /* 1. Derive valid 40-character node ID and export capability frame from KEYSTONE */
    char test_node_id[QIHSE_CLUSTER_NODE_ID_LEN + 1u];
    qihse_cluster_node_id_from_seed("keystone-peer-node-seed", 23, test_node_id);

    uint8_t cap_buf[QIHSE_CLUSTER_BUS_NODE_CAP_PAYLOAD_SIZE];
    int export_len = qihse_fabric_index_export_node_cap(test_node_id, cap_buf, sizeof(cap_buf));
    assert(export_len == (int)QIHSE_CLUSTER_BUS_NODE_CAP_PAYLOAD_SIZE);
    assert(export_len == 50);

    /* Verify wire layout generated by KEYSTONE */
    assert(memcmp(cap_buf, test_node_id, QIHSE_CLUSTER_NODE_ID_LEN + 1u) == 0);
    assert(cap_buf[QIHSE_CLUSTER_NODE_ID_LEN] == '\0');
    uint8_t isa = cap_buf[41];
    uint8_t npu = cap_buf[42];
    uint8_t gpu = cap_buf[43];
    uint32_t ram; memcpy(&ram, cap_buf + 44, 4);
    uint16_t load; memcpy(&load, cap_buf + 48, 2);
    assert(isa <= 4);
    assert(npu <= 1);
    assert(gpu <= 1);
    assert(ram > 0);
    printf("  -> KEYSTONE probed local capabilities: ISA tier=%u, NPU=%u, GPU=%u, Free RAM=%u MB, Load=%u.%02u\n",
           isa, npu, gpu, ram, load / 100, load % 100);

    /* 2. Setup QIHSE cluster topology with 2 nodes */
    qihse_cluster_topology_t* topo = qihse_cluster_topology_create();
    assert(topo != NULL);

    qihse_cluster_node_t local_node;
    memset(&local_node, 0, sizeof(local_node));
    qihse_cluster_node_id_from_seed("qihse-primary", 13, local_node.id);
    snprintf(local_node.host, sizeof(local_node.host), "127.0.0.1");
    local_node.port = 7100;
    local_node.bus_port = 17100;
    local_node.role = QIHSE_CLUSTER_NODE_PRIMARY;
    local_node.healthy = true;
    uint16_t local_idx = 0;
    assert(qihse_cluster_topology_upsert_node(topo, &local_node, &local_idx));
    ...
    /* Peer node corresponding to KEYSTONE node */
    qihse_cluster_node_t peer_node;
    memset(&peer_node, 0, sizeof(peer_node));
    memcpy(peer_node.id, test_node_id, QIHSE_CLUSTER_NODE_ID_LEN + 1u);
    snprintf(peer_node.host, sizeof(peer_node.host), "127.0.0.1");
    peer_node.port = 7101;
    peer_node.bus_port = 17101;
    peer_node.role = QIHSE_CLUSTER_NODE_PRIMARY;
    peer_node.healthy = true;
    uint16_t peer_idx = 0;
    assert(qihse_cluster_topology_upsert_node(topo, &peer_node, &peer_idx));

    /* Create cluster bus on ephemeral/local port */
    qihse_cluster_bus_config_t bus_cfg;
    memset(&bus_cfg, 0, sizeof(bus_cfg));
    bus_cfg.topology = topo;
    bus_cfg.local_node_index = local_idx;
    bus_cfg.bus_port = 0; /* ephemeral */
    bus_cfg.bind_address = "127.0.0.1";
    bus_cfg.heartbeat_ms = 1000;
    bus_cfg.timeout_ms = 5000;

    qihse_cluster_bus_t* bus = qihse_cluster_bus_create(&bus_cfg);
    assert(bus != NULL);

    /* 3. Verify that peer has no capabilities before receiving frame */
    uint8_t out_isa = 0, out_npu = 0, out_gpu = 0;
    uint32_t out_ram = 0;
    uint16_t out_load = 0;
    assert(!qihse_cluster_bus_node_caps(bus, peer_idx, &out_isa, &out_npu, &out_gpu, &out_ram, &out_load));

    /* 4. Build cluster bus datagram carrying the KEYSTONE NODE_CAP payload */
    uint8_t dgram[QIHSE_CLUSTER_BUS_HEADER_SIZE + QIHSE_CLUSTER_BUS_NODE_CAP_PAYLOAD_SIZE];
    uint32_t magic = QIHSE_CLUSTER_BUS_MAGIC;
    uint32_t msg_type = QIHSE_BUS_MSG_NODE_CAP;
    uint32_t sender_index = peer_idx;
    uint32_t plen = QIHSE_CLUSTER_BUS_NODE_CAP_PAYLOAD_SIZE;

    memcpy(dgram, &magic, 4);
    memcpy(dgram + 4, &msg_type, 4);
    memcpy(dgram + 8, &sender_index, 4);
    memcpy(dgram + 12, &plen, 4);
    memcpy(dgram + QIHSE_CLUSTER_BUS_HEADER_SIZE, cap_buf, QIHSE_CLUSTER_BUS_NODE_CAP_PAYLOAD_SIZE);

    /* 5. Inject datagram into QIHSE cluster bus */
    bool injected = qihse_cluster_bus_inject(bus, dgram, sizeof(dgram), "127.0.0.1", 17101);
    assert(injected);

    /* 6. Verify QIHSE cluster bus peer table was updated with KEYSTONE's capability values */
    assert(qihse_cluster_bus_node_caps(bus, peer_idx, &out_isa, &out_npu, &out_gpu, &out_ram, &out_load));
    assert(out_isa == isa);
    assert(out_npu == npu);
    assert(out_gpu == gpu);
    assert(out_ram == ram);
    assert(out_load == load);

    printf("  -> QIHSE Cluster Bus successfully received and ingested KEYSTONE NODE_CAP frame OK\n");
...
int main(void) {
    ...
    test_keystone_node_cap_cluster_bus_integration();
