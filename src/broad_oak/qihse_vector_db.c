...
#ifdef __SSE__
#include <xmmintrin.h>
#define QIHSE_PREFETCH(addr) _mm_prefetch((const char*)(addr), _MM_HINT_T0)
#else
#define QIHSE_PREFETCH(addr) ((void)0)
#endif
...
#define QIHSE_VDB_MAGNITUDE_NAME "vectors.qmag"
#define QIHSE_VDB_GRAPH_NAME "index.qgraph"
#define QIHSE_VDB_INT8_NAME "vectors.qint8"
#define QIHSE_VDB_TIER_NAME "vectors.qtier"
...
static void qihse_vdb_remove_snapshot_files(const char* db_path) {
    ...
    (void)qihse_vdb_remove_file(db_path, QIHSE_VDB_MAGNITUDE_NAME);
    (void)qihse_vdb_remove_file(db_path, QIHSE_VDB_GRAPH_NAME);
    (void)qihse_vdb_remove_file(db_path, QIHSE_VDB_INT8_NAME);
...
static bool qihse_vdb_graph_save(qihse_vector_db_t vdb) {
    ...
    if (!qihse_vdb_path(path, vdb->db_path, QIHSE_VDB_GRAPH_NAME)) {
        return false;
    }
    ...
    qihse_le_write_u32(header + 8u, 1u); /* version */
    ...
    if (write(fd, header, sizeof(header)) != (ssize_t)sizeof(header) ||
        write(fd, payload, payload_size) != (ssize_t)payload_size) {
        close(fd);
        free(payload);
        return false;
    }
    ...
    return close(fd) == 0 && qihse_fsync_dir(vdb->db_path);
}
...
static bool qihse_vdb_graph_load(qihse_vector_db_t vdb) {
    ...
    if (fd < 0) {
        return errno == ENOENT; /* Not an error if sidecar doesn't exist */
    }
    got = read(fd, header, sizeof(header));
    if (got != (ssize_t)sizeof(header) ||
        memcmp(header, QIHSE_VDB_GRAPH_MAGIC, 8u) != 0) {
        close(fd);
        return false;
    }
    ...
    got = read(fd, payload, payload_size);
    close(fd);
    if (got != (ssize_t)payload_size || qihse_fnv1a64(payload, payload_size) != crc) {
        free(payload);
        return false;
    }

    counts_bytes = vdb->live_vectors * sizeof(size_t);
    neighbors_bytes = vdb->live_vectors * vdb->graph_M * sizeof(size_t);
    ...
    vdb->graph_neighbor_counts = (size_t*)malloc(counts_bytes);
    vdb->graph_neighbors = (size_t*)malloc(neighbors_bytes);
    vdb->graph_live_row_map = (size_t*)malloc(map_bytes);
...
static bool qihse_vdb_tier_save(qihse_vector_db_t vdb) {
    ...
    if (!qihse_vdb_path(path, vdb->db_path, QIHSE_VDB_TIER_NAME)) {
        return false;
    }
    ...
    memcpy(payload, vdb->row_tier, tier_bytes);
    memcpy(payload + tier_bytes, vdb->row_access_counts, count_bytes);
...
static bool qihse_vdb_tier_load(qihse_vector_db_t vdb) {
    ...
    got = read(fd, header, sizeof(header));
    if (got != (ssize_t)sizeof(header) || memcmp(header, "QIHSEQTZ", 8u) != 0) {
        close(fd);
        return false;
    }
    generation = qihse_le_read_u64(header + 12u);
...
static bool qihse_vdb_int8_save(qihse_vector_db_t vdb) {
    ...
    if (!qihse_vdb_path(path, vdb->db_path, QIHSE_VDB_INT8_NAME)) {
        return false;
    }
    ...
    memcpy(payload, vdb->int8_dim_min, vdb->int8_dims * sizeof(float));
    memcpy(payload + vdb->int8_dims * sizeof(float), vdb->int8_dim_max, vdb->int8_dims * sizeof(float));
    memcpy(payload + minmax_bytes, vdb->int8_vectors, quantized_bytes);
...
static bool qihse_vdb_int8_load(qihse_vector_db_t vdb) {
    ...
    got = read(fd, header, sizeof(header));
    if (got != (ssize_t)sizeof(header) ||
        memcmp(header, QIHSE_VDB_INT8_MAGIC, 8u) != 0) {
        close(fd);
        return false;
    }
    generation = qihse_le_read_u64(header + 12u);
...
static int qihse_vdb_graph_search(qihse_vector_db_t vdb,
    ...
    size_t M = vdb->graph_M;
    size_t entry = vdb->graph_entry_point;
    size_t* live_map = vdb->graph_live_row_map;
    size_t* visited = NULL;
    size_t visited_count = 0u;
    size_t visited_capacity = 256u;
    size_t* candidates = NULL;
    size_t candidate_count = 0u;
    size_t candidate_capacity;
    qihse_vdb_graph_neighbor_t* beam = NULL;
    ...
    if (!vdb || !query || !query->query_vector || !out_candidates || !out_count ||
        ...
        vdb->graph_status != QIHSE_VDB_GRAPH_VALID ||
        vdb->graph_neighbors == NULL || !live_map) {
        errno = EINVAL;
        return -1;
    }
    ...
    {
        size_t actual_entry = live_map[entry];
        const qihse_index_row_t* row = &vdb->rows[actual_entry];
        const float* vec = qihse_vdb_vector_at(vdb, row);
        if (vec) {
            beam[0].id = entry;
            beam[0].score = qihse_vdb_cosine_similarity(qvec, vec, dims);
            beam_count = 1u;
            qihse_vdb_track_row_access(vdb, actual_entry);
        }
        visited[0] = entry;
        visited_count = 1u;
    }
    ...
    for (;;) {
        bool improved = false;
        /* Collect all unvisited neighbors of current beam */
        for (i = 0u; i < beam_count; i++) {
            size_t node = beam[i].id;
            size_t n;
            for (n = 0u; n < vdb->graph_neighbor_counts[node]; n++) {
                size_t nid = vdb->graph_neighbors[node * M + n];
                size_t v;
                bool is_visited = false;
                for (v = 0u; v < visited_count; v++) {
                    if (visited[v] == nid) {
                        is_visited = true;
                        break;
                    }
                ...
                if (!is_visited) {
                    if (frontier_count >= frontier_capacity) {
                        size_t* nf = (size_t*)realloc(frontier,
                                                       frontier_capacity * 2u * sizeof(size_t));
                        if (!nf) {
                            goto graph_search_fail;
                        }
                        frontier = nf;
                        frontier_capacity *= 2u;
                    }
                    frontier[frontier_count++] = nid;
                    if (visited_count >= visited_capacity) {
                        size_t* nv = (size_t*)realloc(visited,
                                                       visited_capacity * 2u * sizeof(size_t));
                        if (!nv) {
                            goto graph_search_fail;
                        }
                        visited = nv;
        ...
        for (i = 0u; i < frontier_count; i++) {
            size_t actual = live_map[frontier[i]];
            const qihse_index_row_t* row = &vdb->rows[actual];
            const float* vec;
            float score = 0.0f;

            /* Prefetch next frontier node row and vector */
            if (i + 1u < frontier_count) {
                size_t next_actual = live_map[frontier[i + 1u]];
                QIHSE_PREFETCH(&vdb->rows[next_actual]);
                QIHSE_PREFETCH(vdb->vectors + vdb->rows[next_actual].vector_offset);
            }

            vec = qihse_vdb_vector_at(vdb, row);
            if (vec) {
                score = qihse_vdb_cosine_similarity(qvec, vec, dims);
                qihse_vdb_track_row_access(vdb, actual);
            }

            /* Insert into beam if better than worst */
            if (beam_count < beam_capacity) {
                beam[beam_count].id = frontier[i];
                ...
                size_t w = 0u;
                size_t k;
                for (k = 1u; k < beam_count; k++) {
                    if (beam[k].score < beam[w].score) {
                        w = k;
                    }
                }
                if (score > beam[w].score) {
                    beam[w].id = frontier[i];
                    beam[w].score = score;
                    improved = true;
                }
            }
    ...
    /* Build candidate list from beam, mapping dense -> actual row indices */
    candidate_capacity = beam_count;
    candidates = (size_t*)calloc(candidate_capacity, sizeof(size_t));
    if (!candidates) {
        goto graph_search_fail;
    }
    for (i = 0u; i < beam_count; i++) {
        candidates[candidate_count++] = live_map[beam[i].id];
    }
...
static int qihse_vdb_search_graph_candidates(qihse_vector_db_t vdb,
                                              const qihse_vector_query_t* query,
    ...
    if (!vdb || !query || !query->query_vector || !candidate_rows || !results ||
    ...
    for (i = 0u; i < candidate_count; i++) {
        ...
        if (query->user && !qihse_auth_can_access(query->user, row->classification, row->sci_compartment)) {
            continue;
        }
...
static bool qihse_vdb_cache_lookup(
    ...
    hash = qihse_vdb_cache_hash_query(query->query_vector, query->vector_dims, top_k, metric);
...
static bool qihse_vdb_index_rows_are_direct_mappable(void) {
    ...
    return ((const uint8_t*)&endian)[0] == 1u &&
           ...
           offsetof(qihse_index_row_t, classification) == 44u &&
           offsetof(qihse_index_row_t, sci_compartment) == 46u;
}
...
static bool qihse_vdb_load_snapshot(qihse_vector_db_t vdb, bool use_mmap) {
    ...
    qmag_exists = qihse_vdb_exists(vdb->db_path, QIHSE_VDB_MAGNITUDE_NAME);
...
qihse_vector_db_t qihse_vector_db_open(
    ...
    if (file_backed) {
        ...
        /* Load sidecars only if DB state hasn't changed via WAL replay */
        if (vdb->wal_records_replayed == 0u) {
...
int qihse_vector_db_search(
    ...
    if (query && query->query_mode == QIHSE_VDB_QUERY_TRINARY_MAGNITUDE_BYPASS) {
        ...
        size_t candidate_count = 0u;
        size_t top_k = query->top_k > 0u ? query->top_k : 10u;
        size_t ef_search = query->candidate_pool_size > 0u ? query->candidate_pool_size : top_k * 4u;

        if (vdb->graph_status != QIHSE_VDB_GRAPH_VALID) {
            errno = ENOENT;
            return -1;
        }
        ret = qihse_vdb_graph_search(vdb, query, &candidates, &candidate_count, ef_search);
        if (ret < 0) {
            return -1;
        }
        ret = qihse_vdb_search_graph_candidates(vdb, query, candidates, candidate_count,
                                                  results, max_results);
        free(candidates);
...
static int qihse_vdb_search_exact_rows(qihse_vector_db_t vdb,
    ...
    for (i = 0u; i < vdb->total_vectors; i++) {
        ...
        /* Prefetch upcoming row data and vector */
        if (i + 4u < vdb->total_vectors) {
            QIHSE_PREFETCH(&vdb->rows[i + 4u]);
            QIHSE_PREFETCH(vdb->vectors + vdb->rows[i + 4u].vector_offset);
        }
...
bool qihse_vector_db_flush(qihse_vector_db_t vdb) {
    ...
    if (ok) {
        ...
        /* Rebuild graph index sidecar */
        (void)qihse_vdb_graph_build(vdb, 0u, 0u);
...
static bool qihse_vdb_compact_live_rows(qihse_vector_db_t vdb) {
    ...
    for (i = 0u; i < vdb->total_vectors; i++) {
        ...
        *new_row = *old_row;
        ...
        new_row->classification = QIHSE_CLASS_UNCLASSIFIED;
        new_row->sci_compartment = QIHSE_SCI_NONE;
