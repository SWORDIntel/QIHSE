...
_lib.qihse_tsdb_insert.argtypes = [_TimeSeriesDB_p, ctypes.c_uint32, ctypes.c_uint64, ctypes.c_double]
...
_lib.qihse_tsdb_average_range.argtypes = [_TimeSeriesDB_p, ctypes.c_uint64, ctypes.c_uint64]
...
class TimeSeriesDB:
    ...
    def insert(self, series_id: int, timestamp: int, value: float) -> bool:
        return _lib.qihse_tsdb_insert(self._ptr, series_id, timestamp, value)

    def flush(self):
        _lib.qihse_tsdb_compress_flush(self._ptr)

    def average_range(self, start_ts: int, end_ts: int) -> float:
        return _lib.qihse_tsdb_average_range(self._ptr, start_ts, end_ts)
