...
memshadow_hub_fingerprint_manager_t *memshadow_hub_fingerprint_manager_create(const char *storage_path) {
    ...
    // Initialize capacity
    manager->fingerprint_capacity = 100; // Default capacity
    manager->fingerprints = (memshadow_hub_fingerprint_t *)malloc(
        manager->fingerprint_capacity * sizeof(memshadow_hub_fingerprint_t));
...
void memshadow_hub_fingerprint_load_from_file(memshadow_hub_fingerprint_manager_t *manager) {
    ...
    for (uint32_t i = 0; i < header.count && i < manager->fingerprint_capacity; i++) {
        memshadow_hub_fingerprint_t entry;
        if (fread(&entry, sizeof(entry), 1, fp) == 1) {
            /* Copy fingerprint data */
            manager->fingerprints[manager->fingerprint_count] = entry;
            /* Array is already copied by struct assignment above. Just in case: */
            strncpy(manager->fingerprints[manager->fingerprint_count].hub_node_id, entry.hub_node_id, 256);
            manager->fingerprints[manager->fingerprint_count].hub_node_id[255] = '\0';
            manager->fingerprint_count++;
        }
    }
