...
- [ ] **Nothing populates the topology node's federation UUID.** `qihse_cluster_topology_set_node_uuid()` exists but has NO production caller, so the durable path that `FABRIC.CAPS`, the brain and the job model all rely on finds nothing until something fills that mapping. One population site unblocks three consumers that are otherwise correct code reading nothing.
...
- [ ] **The brain cannot resolve a node's identity on its own.** Its observation record now carries each node's UUID (journal format v2, v1 still decodes), so a decision is traceable to a durable record and a replayer can resolve it — but the live path needs the same UUID population as above.
