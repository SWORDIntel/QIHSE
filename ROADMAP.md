...
- [x] **SQL gaps: UPDATE SET, DELETE WHERE, optimizer histograms — FIXED.** `src/tractable/qihse_sql_parser.c` parses structured `UPDATE ... SET` assignments (`ast->set_columns`, `ast->set_values`, `ast->num_set`) refusing zero-assignment updates, and structured `DELETE WHERE` condition parsing (`ast->where_clause`). `src/tractable/qihse_optimizer.c` populates histogram-driven selectivity estimation (`qihse_optimizer_set_column_histogram`, `hist_fraction_below`). Verified by `make test-sql-completeness`.
