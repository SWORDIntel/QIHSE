...
- [ ] **Legacy backup API is an invariant-1 violation.** `include/qihse_backup.h` carries a pre-existing context-free whole-store export API (`qihse_backup_full`, `qihse_backup_incremental`, `qihse_backup_verify`) whose implementation in `src/tractable/qihse_backup.c` is a stub carrying a TODO. No security context, no callers. It must either gain an authenticated context or be removed.
