...
- [ ] **RESTORE the legacy NULL-context finding (superseded by the fix above).** `query.user == NULL` is ACCEPTED and returns rows instead of failing closed; the NULL principal resolves to the active operator (role=0, clearance=0xFFFF) at `src/broad_oak/qihse_vector_db.c:5841-5845`. This is AGENTS.md invariant 1 violated in shipping code. Impact currently bounded because the rows are UNCLASSIFIED. Found by the gold suite; NOT fixed.
