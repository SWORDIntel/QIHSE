...
- [ ] **`ai_fabric.md` overstates two of its five items.** The blanket "items 1-5 are implemented" is stronger than the evidence: item 1 (NODE_CAP) had NO test until W2.4, and item 3 (fabric job model) is much thinner than the spec — `FABRIC.SUBMIT` stores a JSON record and returns a job id but does NOT execute or forward anything, and does not touch `qihse_task_queue.c`. The code comment says "Phase 1: local execution only" and that local execution is not implemented either. Same failure mode as the six documents that claimed nonexistent tests.
