...
- [x] **F8 — Operational hardening.**
  ...
  - [x] Architecture documentation set: 17 federation documents under `docs/architecture/`, indexed from `docs/README.md`, each stating plainly what is implemented and what is not (no consensus yet). The governing brief is filed at `docs/plans/qihse_federation_upgrade_plan_v3.md`.
...
- [x] **Controller-facing client API (v3.md §25).** `include/qihse_controller.h` + `src/controller/qihse_controller.c`: a synchronous RESP client with a bounded decoder (bulk ≤ 16 MiB, ≤ 1 MiB items, depth ≤ 32) and named wrappers for the whole controller surface — node/trust, namespace/group, object CAS, lease/epoch, event journal, resumable watches, conflicts, rejoin/metrics, build, supply chain, provenance, snapshot, schema, security posture. The client confers no authority; the server's system-domain/scope/classification checks apply unchanged. Verified end-to-end by `tests/test_controller_api.c` including the invariant-3 negatives (unauthenticated and tenant principals refused, no data served). *Remaining: Rust and Python controller SDKs.*
